...
def test_valid_email_multiple_users_found_returns_first_user(user_controller, dao_mock):
...
def test_valid_email_multiple_users_found_log_error(user_controller, dao_mock):
    """Test case for a valid email with multiple users found."""
    users = [
        {'email': 'test_2@example.com', 'id': 1},
        {'email': 'test_2@example.com', 'id': 2},
    ...
    # Capture the print output
    with mock.patch('builtins.print') as mock_print:
        user_controller.get_user_by_email('test_2@example.com')
